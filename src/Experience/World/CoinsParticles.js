...
export default class CoinsParticles {
    ...
    update(robotPosition) {
        ...
        // Calcular dirección desde el robot hacia la moneda (una sola vez)
        const directionToCoins = new THREE.Vector3()
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            // Mover partículas desde el robot hacia la moneda (creando un efecto de guía más dinámico)
            ...
            // Mover partículas en la dirección de la moneda con variación
            positions[i3] += directionToCoins.x * baseSpeed + (Math.random() - 0.5) * 0.05
            positions[i3 + 1] += directionToCoins.y * baseSpeed * 0.3 + verticalDrift + 0.02
            positions[i3 + 2] += directionToCoins.z * baseSpeed + (Math.random() - 0.5) * 0.05
            
            // Si la partícula está muy lejos de la moneda o muy cerca, resetearla cerca del robot
            ...
            if (distToTarget < 2 || distToRobot > 200) {
                // Resetear partícula cerca del robot, en dirección a la moneda con variación
                const offsetDistance = 8 + Math.random() * 10
                const offset = directionToCoins.clone().multiplyScalar(offsetDistance)
                
                // Agregar variación perpendicular para efecto más natural
                const perpendicular = new THREE.Vector3(-directionToCoins.z, 0, directionToCoins.x)
